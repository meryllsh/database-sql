...
--4
CREATE PROCEDURE sp_ins_book_details @lastname VARCHAR(20), @firstname VARCHAR(20), @isbn INTEGER, @title VARCHAR(40), @cpright_date DATETIME, @publ_name VARCHAR(30), @sugg_price REAL
AS
BEGIN
SELECT 
...
CREATE PROCEDURE sp_upd_price @firstname VARCHAR(30), @lastname VARCHAR(30), @x INTEGER
AS
BEGIN
SELECT (@)
...
CREATE PROCEDURE sp_upd_price @Afname VARCHAR(30) , @Alname VARCHAR(30) , @Iperc FLOAT
AS
BEGIN
 UPDATE publisher
...
--EXEC sp_if_upd_price  'Margaret W.','Brown';
